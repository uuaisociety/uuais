...
// reactflow + dagre are heavy; load them only when the connections panel renders.
const CourseConnectionsFlow = dynamic(() => import("./CourseConnectionsFlow"), {
  ssr: false,
  loading: () => (
                  ...
                  {fact.hint && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          type="button"
                          className="-m-3.5 inline-flex items-center justify-center p-3.5 text-gray-500 hover:text-primary transition-colors"
            ...
            <div className="flex flex-wrap gap-3">
              {resourceLinks.map((resource) => (
                <a
                  ...
                  className="inline-flex items-center rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:border-primary hover:text-primary transition-colors"
                >
                  {resource.label}
                </a>
              ))}
