...
const BlogDetailPage: React.FC<BlogDetailPageProps> = ({ postId }) => {
  ...
  // reviewerName falls back to the account email, so strip anything after @
  // before deriving the uuais.com address from the display name.
  const reviewerEmail = (name: string): string =>
    name
      .toLowerCase()
      .replace(/@.*$/, '')
      .replace(/[^a-z0-9]+/g, '.')
          ...
          {blogPost.authorType === 'ai' && (
            <div className="border border-primary/15 bg-primary/[0.04] text-muted-foreground px-4 py-3 rounded-lg flex items-start gap-3 mt-6">
              ...
              <p className="text-sm leading-relaxed">
                Generated by AI with the UU AI Society AI News Desk. Reviewed by{' '}
                {blogPost.reviewedBy ? (
                  <a
                    href={`mailto:${reviewerEmail(blogPost.reviewedBy)}@uuais.com`}
