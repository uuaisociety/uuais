...
```bash
# 1. Create and activate a venv (PEP 668 blocks global pip install)
python3 -m venv .venv-browser-use && source .venv-browser-use/bin/activate

# 2. Install browser-use AND playwright (both are needed)
pip install browser-use playwright

# 3. Install Playwright Chromium browser (required for headless browsing)
...
If `browser-use doctor` shows missing dependencies, install them:
- `cloudflared` — needed for `browser-use tunnel`: https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/
- `profile-use` — needed for Chrome profile management: `browser-use profile update`

**Important**: All `browser-use` commands must be run inside the venv where it's installed. Always prefix with `source .venv-browser-use/bin/activate && browser-use ...` or run the full path like `./.venv-browser-use/bin/browser-use ...`.
...
For advanced browser control (CDP, device emulation, tab activation), see https://github.com/browser-use/browser-use/blob/main/browser_use/skill_cli/README.md
...
When testing a local dev server (e.g., Next.js), the server must stay alive between browser-use commands. Use `nohup` to keep it running:

```bash
# Start dev server persistently
nohup npm run dev > /tmp/nextdev.log 2>&1 &

# Wait for it to be ready
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000   # Should return 200

# Now browse with browser-use
browser-use open http://localhost:3000
```

If the dev server dies, `browser-use open` returns `net::ERR_CONNECTION_REFUSED`. Kill and restart: `kill $(lsof -t -i:3000) 2>/dev/null`

### Exposing Local Dev Servers via Tunnel
...
- **`net::ERR_CONNECTION_REFUSED`?** The target server isn't running. Start it first (e.g., `nohup npm run dev &`) and verify with `curl` before calling `browser-use open`.
- **`playwright` command not found?** Install it explicitly: `pip install playwright && python -m playwright install chromium`.
- **`pip install` fails with PEP 668 error?** You must use a venv. Create one: `python3 -m venv .venv-browser-use && source .venv-browser-use/bin/activate`.
